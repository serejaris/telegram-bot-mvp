...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Получаем всё в одном запросе с помощью CTE и LATERAL JOIN
        # для устранения проблемы N+1
        await cur.execute("""
            WITH chat_counts AS (
                SELECT
                    c.id,
            ...
            LEFT JOIN LATERAL (
                SELECT json_agg(
                    json_build_object('name', sub.name, 'count', sub.count)
        ...
        result = []
        for row in rows:
            # Parsing logic for json (fallback if not automatically parsed)
