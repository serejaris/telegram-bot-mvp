...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Optimized query using CTE and LATERAL joins to fetch everything in one go
        await cur.execute("""
            WITH stats AS (
                SELECT
                    chat_id,
            ...
            SELECT
                ...
                COALESCE(s.today, 0) as today_messages,
                lm.text,
                COALESCE(lu.username, lu.first_name, 'Unknown') as last_message_author,
                lm.sent_at,
                COALESCE(tu.data, '[]'::json) as top_users
            FROM chats c
            LEFT JOIN stats s ON c.id = s.chat_id
            LEFT JOIN LATERAL (
