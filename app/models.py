    ...
    async with get_cursor() as cur:
        await cur.execute("""
            ...
            top_users_agg AS (
                SELECT
                    chat_id,
                    json_agg(json_build_object('name', name, 'count', count) ORDER BY count DESC) as users
            ...
            LEFT JOIN top_users_agg tu ON cs.id = tu.chat_id
        ...
        for row in rows:
            # Handle top_users which might be a list (if driver decodes JSON) or string
            top_users_raw = row[7]
