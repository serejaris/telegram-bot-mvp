...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    Оптимизировано: используется один запрос с CTE и LEFT JOIN LATERAL вместо N+1 запросов.
    """
    async with get_cursor() as cur:
        await cur.execute("""
            WITH ChatStats AS (
            ...
            LEFT JOIN LATERAL (
                ...
                FROM messages m
                LEFT JOIN users u ON m.user_id = u.id
                WHERE m.chat_id = cs.id AND m.text IS NOT NULL
        ...
        result = []
        for row in rows:
            # Parse top_users JSON if it's a string (depends on driver/mock)
            top_users_data = row[7]
            if top_users_data and isinstance(top_users_data, str):
                try:
