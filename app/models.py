...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Optimized query using CTEs and LATERAL JOINs to prevent N+1 queries
        await cur.execute("""
            WITH chat_counts AS (
                SELECT
                    c.id,
            ...
            LEFT JOIN LATERAL (
                ...
                FROM messages m
                LEFT JOIN users u ON m.user_id = u.id
                WHERE m.chat_id = cc.id AND m.text IS NOT NULL
                ORDER BY m.sent_at DESC
                LIMIT 1
            ) lm ON TRUE
            LEFT JOIN LATERAL (
                SELECT
                    json_agg(json_build_object('name', sub.name, 'count', sub.count) ORDER BY sub.count DESC) as users_list
                FROM (
                    SELECT
            ...
            ORDER BY cc.total_messages DESC
        """)
        ...
        result = []
        for row in rows:
            # Handle potential string vs list for json field depending on driver config
            top_users = row[7]
