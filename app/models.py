...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Optimized query using CTE and LATERAL JOIN to avoid N+1 problem
        await cur.execute("""
            WITH chat_stats AS (
                SELECT
                    chat_id,
                    COUNT(message_id) as total_messages,
                    COUNT(message_id) FILTER (WHERE sent_at >= CURRENT_DATE) as today_messages
                FROM messages
                GROUP BY chat_id
            )
            SELECT
                ...
                COALESCE(cs.today_messages, 0) as today_messages,
                last_msg.text as last_message_text,
                last_msg.author as last_message_author,
        ...
        result = []
        for row in rows:
            # Parse top_users if it comes as a string (depends on driver/mock)
            # psycopg 3 often returns json as python objects automatically
            top_users = row[7]
            ...
            result.append(DashboardChat(
                ...
                last_message_at=row[6],
                top_users=top_users if top_users is not None else [],
            ))
