...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Используем CTE и LATERAL JOIN для оптимизации запроса (N+1 проблема)
        await cur.execute("""
            SELECT
                c.id,
                c.title,
            ...
            LEFT JOIN LATERAL (
                SELECT
                    json_agg(json_build_object('name', t.name, 'count', t.count)) as top_users
                FROM (
                    SELECT
            ...
            GROUP BY c.id, c.title, lm.text, lm.author, lm.sent_at, tu.top_users
        ...
        result = []
        for row in rows:
            # psycopg автоматически десериализует JSON, но на случай использования других драйверов
            # или моков, проверим тип
            top_users = row[7]
