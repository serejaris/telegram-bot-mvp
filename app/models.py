...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Получаем всё одним запросом используя CTE и LATERAL JOIN
        await cur.execute("""
            WITH chat_stats AS (
                SELECT
                    c.id,
            ...
            SELECT
                ...
                cs.today_messages,
                lm.text,
                COALESCE(lm_u.username, lm_u.first_name, 'Unknown'),
                lm.sent_at,
                COALESCE(tu.users_json, '[]'::json)
            FROM chat_stats cs
            LEFT JOIN LATERAL (
                SELECT m.text, m.user_id, m.sent_at
                FROM messages m
        ...
        result = []
        for row in rows:
            # row[7] - список словарей (авто-десериализация JSON в psycopg)
            # но на случай если придет строка (например в тестах), обработаем это
            top_users = row[7]
