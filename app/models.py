...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Оптимизированный запрос с использованием CTE и JOIN LATERAL
        # для решения проблемы N+1 запросов
        await cur.execute("""
            WITH chat_stats AS (
                SELECT
                    c.id,
            ...
            LEFT JOIN LATERAL (
                SELECT
                    json_agg(
                        json_build_object('name', t.name, 'count', t.count)
                    ) as data
                FROM (
                    SELECT
        ...
        for row in rows:
            ...
            if top_users_data is None:
                top_users = []
            elif isinstance(top_users_data, str):
                top_users = json.loads(top_users_data)
            else:
