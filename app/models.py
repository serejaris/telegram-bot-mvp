...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Оптимизация: используем один сложный запрос вместо N+1 запросов
        await cur.execute("""
            WITH chats_base AS (
                SELECT
                    c.id,
            ...
            FROM chats_base cb
            LEFT JOIN last_messages lm ON cb.id = lm.chat_id
            LEFT JOIN top_users_agg tu ON cb.id = tu.chat_id
            ORDER BY cb.total_messages DESC
        """)
        rows = await cur.fetchall()
        ...
        return [
            DashboardChat(
                ...
                last_message_at=row[6],
                # Обработка json: psycopg может вернуть список или строку
                top_users=row[7] if isinstance(row[7], list) else (json.loads(row[7]) if isinstance(row[7], str) else [])
            )
            for row in rows
        ]
