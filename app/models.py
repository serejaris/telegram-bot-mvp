    ...
    async with get_cursor() as cur:
        await cur.execute("""
            ...
            top_users_agg AS (
                SELECT
                    chat_id,
                    json_agg(json_build_object('name', name, 'count', count) ORDER BY count DESC) as top_users
        ...
        for row in rows:
            top_users = row[7]
            # Handle potential string return from database driver
