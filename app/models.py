...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Optimizing N+1 query problem using CTEs and LATERAL joins
        # This reduces the number of queries from 1 + 2*N to 1
        await cur.execute("""
            WITH chat_stats AS (
                SELECT
                    c.id,
            ...
            LEFT JOIN LATERAL (
                SELECT json_agg(t ORDER BY t.count DESC) as users
                FROM (
                    SELECT
