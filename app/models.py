...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Optimization: Fetch all data in a single query using CTEs and LATERAL joins
        # to avoid N+1 problem (1 query instead of 1 + 2N).
        await cur.execute("""
            SELECT
                cs.id,
                cs.title,
            ...
            LEFT JOIN LATERAL (
                SELECT
                    m2.text,
                    COALESCE(u2.username, u2.first_name, 'Unknown') as author,
                    m2.sent_at
                FROM messages m2
            ...
            LEFT JOIN LATERAL (
                SELECT
                    json_agg(json_build_object('name', sub.name, 'count', sub.count) ORDER BY sub.count DESC) as top_users
                FROM (
                    SELECT
                        COALESCE(u3.username, u3.first_name, 'Unknown') as name,
                        COUNT(*) as count
                    FROM messages m3
                    JOIN users u3 ON m3.user_id = u3.id
                    WHERE m3.chat_id = cs.id
                      AND m3.sent_at >= NOW() - INTERVAL '7 days'
                    GROUP BY u3.id, u3.username, u3.first_name
                    ORDER BY count DESC
                    LIMIT 3
                ) sub
            ) tu ON TRUE
        ...
        result = []
        for row in rows:
            # Handle potential JSON serialization if driver returns string
            top_users = row[7]
