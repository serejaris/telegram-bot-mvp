...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    Optimized to use a single query with LEFT JOIN LATERAL to avoid N+1 problem.
    """
    async with get_cursor() as cur:
        await cur.execute("""
            SELECT
            ...
            LEFT JOIN LATERAL (
                ...
                FROM messages m
                LEFT JOIN users u ON m.user_id = u.id
                WHERE m.chat_id = c.id AND m.text IS NOT NULL
                ORDER BY m.sent_at DESC
                LIMIT 1
            ) last_msg ON TRUE
            LEFT JOIN LATERAL (
                SELECT
                    json_agg(json_build_object('name', t.name, 'count', t.count)) as users
                FROM (
                    SELECT
            ...
            ORDER BY stats.total_messages DESC;
        """)
        ...
        for row in chats_data:
            top_users = row[7]
            # json_agg can return None if no rows match, but COALESCE handles it.
            # However, if t is empty, json_agg returns NULL (which we coalesced to '[]').
            # psycopg might return it as a list of dicts directly if json adapter is working,
            # or as a string/json object.
            # Assuming psycopg returns decoded JSON (list of dicts) for json columns.
