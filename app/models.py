...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    Оптимизировано для использования одного SQL запроса вместо N+1.
    """
    async with get_cursor() as cur:
        await cur.execute("""
            SELECT
            ...
            LEFT JOIN LATERAL (
                ...
                WHERE m_last.chat_id = stats.id AND m_last.text IS NOT NULL
                ORDER BY m_last.sent_at DESC
                LIMIT 1
            ) lm ON TRUE
            LEFT JOIN LATERAL (
                SELECT json_agg(top) as users_json
                FROM (
                    SELECT
