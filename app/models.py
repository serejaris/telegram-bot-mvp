    ...
    async with get_cursor() as cur:
        await cur.execute("""
            ...
            top_users_per_chat AS (
                SELECT
                    ...
                    COUNT(*) as count,
                    ROW_NUMBER() OVER (PARTITION BY m.chat_id ORDER BY COUNT(*) DESC) as rank
                FROM messages m
            ...
            LEFT JOIN top_users_agg tua ON cs.id = tua.chat_id
        ...
        return [
            DashboardChat(
                ...
                last_message_at=row[6],
                top_users=row[7] if isinstance(row[7], list) else (
                    json.loads(row[7]) if isinstance(row[7], str) else []
                ),
            )
            for row in rows
        ]
