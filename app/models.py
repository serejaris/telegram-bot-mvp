...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    Optimized: Uses CTE and LATERAL joins to fetch all data in a single query
    ...
    async with get_cursor() as cur:
        ...
        await cur.execute("""
            ...
            SELECT
                ...
                last_msg.author,
                last_msg.sent_at,
                COALESCE(top_users.data, '[]'::json) as top_users_data
            FROM chat_stats cs
            LEFT JOIN LATERAL (
                SELECT
                    m.text,
        ...
        return [
            DashboardChat(
                ...
                last_message_at=row[6],
                top_users=row[7] if isinstance(row[7], list) else json.loads(row[7]) if row[7] else [],
            )
            for row in rows
        ]
