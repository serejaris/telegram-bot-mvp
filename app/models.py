...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    Оптимизировано для выполнения в один SQL-запрос для избежания проблемы N+1.
    """
    async with get_cursor() as cur:
        await cur.execute("""
            WITH chat_stats AS (
            ...
            top_users AS (
                SELECT
                    chat_id,
                    json_agg(json_build_object('name', name, 'count', count) ORDER BY count DESC) as top_users_json
                FROM top_users_base
                WHERE rn <= 3
                GROUP BY chat_id
            )
            SELECT
                ...
                COALESCE(tu.top_users_json, '[]'::json) as top_users
            FROM chat_stats cs
            LEFT JOIN last_messages lm ON cs.id = lm.chat_id
            LEFT JOIN top_users tu ON cs.id = tu.chat_id
        ...
        result = []
        for row in rows:
            # Обработка top_users на случай если драйвер вернул строку JSON или список
