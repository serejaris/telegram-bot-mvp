...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Optimized query using LATERAL joins to fetch everything in one go
        await cur.execute("""
            SELECT
                c.id,
                c.title,
                ...
                last_msg.author,
                last_msg.sent_at,
                COALESCE(top_users.data, '[]'::json) as top_users
            FROM chats c
            LEFT JOIN messages m ON c.id = m.chat_id
            LEFT JOIN LATERAL (
                SELECT
                    m_last.text,
                    COALESCE(u_last.username, u_last.first_name, 'Unknown') as author,
                    m_last.sent_at
                FROM messages m_last
                LEFT JOIN users u_last ON m_last.user_id = u_last.id
                WHERE m_last.chat_id = c.id AND m_last.text IS NOT NULL
                ORDER BY m_last.sent_at DESC
                LIMIT 1
            ) last_msg ON TRUE
            LEFT JOIN LATERAL (
                SELECT
                    json_agg(json_build_object('name', t.name, 'count', t.count) ORDER BY t.count DESC) as data
                FROM (
                    SELECT
                        COALESCE(u_top.username, u_top.first_name, 'Unknown') as name,
                        COUNT(*) as count
                    FROM messages m_top
                    JOIN users u_top ON m_top.user_id = u_top.id
                    WHERE m_top.chat_id = c.id
                      AND m_top.sent_at >= NOW() - INTERVAL '7 days'
                    GROUP BY u_top.id, u_top.username, u_top.first_name
                    ORDER BY count DESC
                    LIMIT 3
                ) t
            ) top_users ON TRUE
            GROUP BY c.id, c.title, last_msg.text, last_msg.author, last_msg.sent_at, top_users.data
        ...
        for row in chats_data:
            # Handle top_users: psycopg might return list/dict or json string
            top_users_data = row[7]
