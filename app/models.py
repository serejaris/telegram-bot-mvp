    ...
    async with get_cursor() as cur:
        ...
        await cur.execute("""
            SELECT
                ...
                lm.author,
                lm.sent_at,
                COALESCE(tu.users, '[]'::json) as top_users
            FROM chats c
            LEFT JOIN (
                SELECT
                    chat_id,
            ...
            LEFT JOIN LATERAL (
                SELECT
                    json_agg(json_build_object('name', sub.name, 'count', sub.count) ORDER BY sub.count DESC) as users
                FROM (
                    SELECT
        ...
        result = []
        for row in rows:
            # psycopg 3 automatically deserializes JSON, but we handle string case just in case
            top_users = row[7]
