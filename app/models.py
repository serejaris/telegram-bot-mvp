...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        ...
        # Это решает проблему N+1 (100+ запросов для 50 чатов)
        await cur.execute("""
            WITH chat_stats AS (
                SELECT
                    c.id,
        ...
        result = []
        for row in rows:
            # psycopg 3 автоматически десериализует JSONB в Python объекты
            # Но если данные пришли строкой (например, из-за настроек драйвера),
            # используем json.loads
