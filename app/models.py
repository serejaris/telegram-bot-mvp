...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Fetch everything in a single optimized query using LATERAL joins
        # This prevents N+1 query problem (1 query instead of 1 + 2*N queries)
        await cur.execute("""
            SELECT
                sub.id,
                sub.title,
            ...
            LEFT JOIN LATERAL (
                ...
                FROM messages m
                LEFT JOIN users u ON m.user_id = u.id
                WHERE m.chat_id = sub.id AND m.text IS NOT NULL
                ORDER BY m.sent_at DESC
                LIMIT 1
            ) lm ON TRUE
            LEFT JOIN LATERAL (
                SELECT
                    json_agg(json_build_object('name', t.name, 'count', t.count) ORDER BY t.count DESC) as top_users
                FROM (
                    SELECT
            ...
            ORDER BY sub.total_messages DESC
        """)
        rows = await cur.fetchall()

        result = []
        for row in rows:
            # Handle top_users which might be returned as string or list depending on driver/mock
            top_users = row[7]
