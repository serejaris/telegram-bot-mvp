...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Используем LEFT JOIN LATERAL для оптимизации (N+1 проблема)
        await cur.execute("""
            SELECT
                c.id,
                c.title,
                COALESCE(stats.total_messages, 0) as total_messages,
                COALESCE(stats.today_messages, 0) as today_messages,
                lm.text as last_message_text,
                lm.author as last_message_author,
                lm.sent_at as last_message_at,
                COALESCE(tu.top_users, '[]'::json) as top_users
            FROM chats c
            LEFT JOIN LATERAL (
                SELECT
            ...
            LEFT JOIN LATERAL (
                ...
                LEFT JOIN users u ON m_last.user_id = u.id
            ...
            LEFT JOIN LATERAL (
                SELECT
                    json_agg(json_build_object('name', sub.name, 'count', sub.count)) as top_users
                FROM (
                    SELECT
                        COALESCE(u.username, u.first_name, 'Unknown') as name,
                        COUNT(*) as count
                    FROM messages m_top
                    JOIN users u ON m_top.user_id = u.id
        ...
        return [
            DashboardChat(
                ...
                last_message_at=row[6],
                top_users=row[7] if isinstance(row[7], list) else json.loads(row[7]),
            )
            for row in rows
        ]
