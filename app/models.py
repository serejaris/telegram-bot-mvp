...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    # ⚡ Bolt: Optimized to use a single query instead of N+1 queries.
    # Uses LATERAL joins to fetch per-chat aggregated data efficiently.
    async with get_cursor() as cur:
        await cur.execute("""
            SELECT
                c.id,
            ...
            LEFT JOIN LATERAL (
                SELECT
                    ...
                    COUNT(*) FILTER (WHERE m.sent_at >= CURRENT_DATE) as today_messages
                FROM messages m
            ...
            LEFT JOIN LATERAL (
                SELECT json_agg(json_build_object('name', t.name, 'count', t.count)) as data
                FROM (
                    SELECT
