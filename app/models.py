...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Оптимизированный запрос с использованием CTE для извлечения всех данных одним запросом
        await cur.execute("""
            WITH chat_stats AS (
                SELECT
                    c.id,
            ...
            top_users AS (
                SELECT
                    chat_id,
                    json_agg(json_build_object('name', name, 'count', count) ORDER BY count DESC) as top_users_list
                FROM (
                    SELECT
                        chat_id,
                        name,
            ...
            SELECT
                ...
                lm.author,
                lm.sent_at,
                COALESCE(tu.top_users_list, '[]'::json)
            FROM chat_stats cs
