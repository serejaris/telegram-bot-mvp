...
async def get_dashboard_data() -> List[DashboardChat]:
    ...
    async with get_cursor() as cur:
        # Optimization: Fetch all data in a single query using CTE and LATERAL joins
        # to avoid N+1 query problem (1 + 2N queries previously).
        await cur.execute("""
            WITH chat_stats AS (
                SELECT
                    c.id,
            ...
            LEFT JOIN LATERAL (
                SELECT json_agg(json_build_object('name', sub.name, 'count', sub.count) ORDER BY sub.count DESC) as users_json
                FROM (
                    SELECT
